export const SearchContent: React.FC = () => {
    return (
        <div className="w-full flex flex-col mt-2 bg-black py-1 absolute top-full left-0 border-2 border-black-2.5 rounded-md">
            <div className="flex flex-col">
                {SEARCH_CONTENTS.map(({ title, thumbnail }) => (
                    <div
                        key={title}
                        className="text-white/85 py-1 pr-1 pl-2 cursor-pointer flex items-center gap-2 hover:bg-white/10"
                    >
